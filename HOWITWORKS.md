...
-   Blocking Pop
    1.  `Worker->reserve()` calls `Resque_Job::reserveBlocking()` with
...
3.  In either case, `Worker->reserve()` returns the new `Resque_Job`
...
-   Job Reserved
    ...
    3.  `Worker->workingOn()` does some reference assignments to help
        keep track of the worker/job relationship, then updates the job status
        from `WAITING` to `RUNNING`
    4.  `Worker->workingOn()` stores the new `Resque_Job` object's
        ...
        returns control to `Worker::work()`
    5.  `Worker::work()` forks a child process to run the actual
        `perform()`
    ...
    -   Worker
        ...
        2.  If the exit status is not 0, the worker calls `Resque_Job->fail()`
            with a `DirtyExitException` as its only argument.
        ...
        5.  `Resque_Job->fail()` calls `Resque_Failure::create()` with the job
            payload, the `DirtyExitException`, the internal ID of the
    ...
    -   Job
        1.  `PID` is created, registering the PID of the actual process
            doing the job.
        2.  The job calls `Worker->perform()` with the `Resque_Job` as
            its only argument.
        3.  `Worker->perform()` sets up a `try...catch` block so it can
            properly handle exceptions by marking jobs as failed (by calling
            `Resque_Job->fail()`, as above)
        4.  Inside the `try...catch`, `Worker->perform()` triggers an
            `afterFork` event
        5.  Still inside the `try...catch`, `Worker->perform()` calls
        ...
        18. The `try...catch` block in `Worker->perform()` ends
        19. `Worker->perform()` updates the job status from `RUNNING` to
            `COMPLETE`, then returns control, with no value, to the worker
            (again still in `Worker::work()`)
        20. `PID()` is removed, the forked process will terminate soon
            cleanly
        21. `Worker::work()` calls `exit(0)` to terminate the job process
    ...
    7.  `Worker::work()` calls `Worker->doneWorking()` with no
        arguments
    8.  `Worker->doneWorking()` increments two processed counters in
